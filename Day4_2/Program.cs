...
namespace Day4_2
{
	class Program
	{
		...
		{
			var passphrases = GetInput().Replace("\r", "").Replace("\n", "|")
										.Split('|');

			int validSum = 0;
			foreach (var p in passphrases)
			{
				var words = p.Split(' ');
				if (IsValid(words))
					validSum++;
		...
		private static bool IsValid(string[] words)
		{
			...
			for (int i = 0; i < words.Length; i++)
			{
				...
				{
					var compare = new List<char>(words[j].ToArray());

					if(current.Length == compare.Count)
					{
						...
						if (compare.Count == 0)	//all has been found and removed
							return false;
					}
				}
			}
