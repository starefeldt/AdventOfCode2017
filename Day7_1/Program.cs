...
namespace Day7_1
{
	...
	class Program
	{
		...
		{
			var data = GetProgramData().Replace("\r", "")
									   .Replace("\n", "|")
									   .Split('|');
			...
			foreach (var tower in towers)
			{
				...
				if (tower.Children.Count != 0)
				{
					...
					{
						if (tower.Name != compare.Name)
						{
							var result = compare.Children.FirstOrDefault(x => x == tower.Name);
							if (result != null)
								count++;
						}
					}
		...
		private static List<TowerProgram> GetTowers(string[] data)
		{
			...
			foreach (var line in data)
			{
				...
				if (childrenStart != -1)
				{
					children = line.Substring(childrenStart + 3)
									.Replace(",", "")
									.Split(' ').ToList();
				}
