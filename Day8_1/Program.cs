...
namespace Day8_1
{
	...
	class Program
	{
		static Dictionary<string, int> _sources = new Dictionary<string, int>();
		...
		static void Main(string[] args)
		{
			...
			{
				if (IsConditionTrue(row.Condition))
				{
					int value = _sources[row.Source];
					switch (row.Instruction.ChangeValue)
					{
						case ChangeValue.Increment:
							value += row.Instruction.Value;
							break;
						case ChangeValue.Decrement:
							value -= row.Instruction.Value;
							break;
					}
					_sources[row.Source] = value;
				}
			}
			int result = _sources.Values.Max();
			Console.WriteLine(result);
			Console.ReadLine();
		}

		private static bool IsConditionTrue(Condition condition)
		{
			int value = _sources[condition.Source];
			switch (condition.ConditionValue)
			{
				case ConditionValue.Gt:
					return value > condition.Value;
				case ConditionValue.Lt:
					return value < condition.Value;
				case ConditionValue.Equal:
					return value == condition.Value;
				case ConditionValue.NotEqual:
					return value != condition.Value;
				case ConditionValue.GtOrEqual:
					return value >= condition.Value;
				case ConditionValue.LtOrEqual:
					return value <= condition.Value;
			}
			return false;
		}

		private static List<RowData> CreateRowData(string data)
		{
			var allRowData = new List<RowData>();
			data = data.Replace("\r\n", "|");
			var inputAsRows = data.Split('|');

			foreach (var row in inputAsRows)
			{
				var firstSpace = row.IndexOf(' ');
				var source = row.Remove(firstSpace);
				TryAddSource(source);

				var rowData = new RowData();
				rowData.Source = source;

				var conditionStart = row.IndexOf("if") - 1;
				rowData.Instruction = GetInstruction(row, firstSpace, conditionStart);
				rowData.Condition = GetCondition(row, conditionStart);
				allRowData.Add(rowData);
			}
			return allRowData;
		}

		private static void TryAddSource(string source)
		{
			if (!_sources.ContainsKey(source))
				_sources.Add(source, 0);
		}

		private static Condition GetCondition(string row, int startIndex)
		{
			var cond = new Condition();
			var condData = row.Substring(startIndex + 4).Split(' ');    //Do not want word "if"
			var source = condData[0];
			TryAddSource(source);
			cond.Source = source;
			cond.ConditionValue = GetConditionValue(condData[1]);
			cond.Value = int.Parse(condData[2]);
			return cond;
		}

		private static ConditionValue GetConditionValue(string condValue)
		{
			switch (condValue)
			{
				case ">":
				...
				case "==":
					return ConditionValue.Equal;
				case "!=":
					return ConditionValue.NotEqual;
				case ">=":
					return ConditionValue.GtOrEqual;
				case "<=":
					return ConditionValue.LtOrEqual;
			}
			throw new ArgumentException();
		}

		private static Instruction GetInstruction(string row, int startIndex, int endIndex)
		{
			var instr = new Instruction();
			var instructionData = row.Substring(startIndex + 1, endIndex - startIndex).Trim()
									 .Split(' ');

			if (instructionData[0] == "inc")
				instr.ChangeValue = ChangeValue.Increment;
			else
				instr.ChangeValue = ChangeValue.Decrement;
			instr.Value = int.Parse(instructionData[1]);
			return instr;
		}

		private static string GetData()
		{
			return @"ebu inc 626 if iq < 0
